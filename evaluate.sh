...
if  [ ${DATASET} == "caption-vizwiz-val" ]; then
  torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/caption/evaluate_caption.py --checkpoint ${CHECKPOINT} --datasets vizwiz-caption-val "${ARGS[@]:2}"
fi

if  [ ${DATASET} == "caption-textcaps-val" ]; then
  torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/caption/evaluate_caption.py --checkpoint ${CHECKPOINT} --datasets textcaps-val "${ARGS[@]:2}"
fi

if [ ${DATASET} == "grouding-skvg-test" ]; then
    torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/refcoco/evaluate_grounding.py --checkpoint ${CHECKPOINT} --datasets skvg_test "${ARGS[@]:2}"
fi

if [ ${DATASET} == "vqa-iconqa-test" ]; then
    torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/vqa/evaluate_multi_choice.py --checkpoint ${CHECKPOINT} --datasets iconqa_test "${ARGS[@]:2}"
fi

if [ ${DATASET} == "vqa-pmcvqa-test-clean" ]; then
    torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/vqa/evaluate_multi_choice.py --checkpoint ${CHECKPOINT} --datasets pmcvqa_test_clean "${ARGS[@]:2}"
fi

if [ ${DATASET} == "vqa-kvqa-test" ]; then
    torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/vqa/evaluate_vqa.py --checkpoint ${CHECKPOINT} --datasets kvqa_test "${ARGS[@]:2}"
fi
...
ds_collections = {
    ...
    "skvg_test": "data/sk-vg/test.jsonl",
}
    ...
    def __getitem__(self, idx):
        ...
        return {
            "text": (
                ...
                if "knowledge" not in self.prompt
                else self.prompt.format(knowledge, text)
            ),
...
def evaluate_chat_model():
    ...
    for ds_name in args.datasets:
        ...
        outputs = []
        for _, (pixel_values, questions, bboxes, hws, num_patches_list) in enumerate(
            tqdm(dataloader)
        ):
        ...
        if torch.distributed.get_rank() == 0:
            ...
            for i, output in enumerate(merged_outputs):
                ...
                target_bbox = torch.tensor(output["gt_bbox"], dtype=torch.float32).view(
                    -1, 4
                )
                predict_bbox = torch.tensor(predict_bbox, dtype=torch.float32).view(
                    -1, 4
                )
            ...
            summaries.append(
                [args.checkpoint, ds_name, f"Precision @ 1: {correct / total_cnt} \n"]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "refcocog_val,refcocog_test,skvg_test",
    )

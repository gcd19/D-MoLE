...
ds_collections = {
    ...
    "vizwiz-caption-val": {
        "root": "data/vizwiz-caption/images/val",
        "annotation": "data/vizwiz-caption/val.json",
        "max_new_tokens": 30,
        "min_new_tokens": 8,
    },
    "textcaps-val": {
        "root": "data/textcaps/images",
        "annotation": "data/textcaps/val.json",
        "max_new_tokens": 30,
        "min_new_tokens": 8,
    },
}
...
def evaluate_chat_model():
    ...
    for ds_name in args.datasets:
        ...
        image_ids, captions = [], []
        for _, (pixel_values, ids, _, _, num_patches_list) in enumerate(
            tqdm(dataloader, disable=torch.distributed.get_rank() != 0)
        ):
            ...
            preds = model.batch_chat(
                ...
                questions=[prompt] * len(pixel_values),
        ...
        rank0_print(f"Average caption length: {average_length}")

        if torch.distributed.get_rank() == 0:
            rank0_print(f"Evaluating {ds_name} ...")
...
if __name__ == "__main__":
    ...
    rank0_print("datasets:", args.datasets)
    ...
    if total_params > 20 or args.dynamic:
        args.num_beams = 1
        rank0_print(f"[test] total_params: {total_params}B, use num_beams: {args.num_beams}")
    else:
        rank0_print(f"[test] total_params: {total_params}B")
    rank0_print(f"[test] image_size: {image_size}")
    rank0_print(f"[test] template: {model.config.template}")
    rank0_print(f"[test] dynamic_image_size: {args.dynamic}")
    rank0_print(f"[test] use_thumbnail: {use_thumbnail}")
    rank0_print(f"[test] max_num: {args.max_num}")

...
def main():
    ...
    # Only wrap LoRA if not already initialized (e.g., when loading from base model)
    if model_args.use_backbone_lora and not hasattr(model.vision_model, "peft_config"):
        model.wrap_backbone_lora(r=model_args.use_backbone_lora, lora_alpha=2 * model_args.use_backbone_lora)
        model.config.use_backbone_lora = model_args.use_backbone_lora

    if model_args.use_llm_lora and not hasattr(model.language_model, "peft_config"):
